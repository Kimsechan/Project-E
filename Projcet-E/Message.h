...
enum class MessageType
{
	...
	Length 
	//제가 가진 메세지 타입의 개수보다 더 많은 내용이 들어오면 무시
};
...
void BroadCastMessage(char* message, int length, int sendFD = -1, bool sendSelf = true)
{
	...
	//굳이 뭐.. 더 돌 필요 있나요
	//보낼 때 마다 send를 1 씩 늘려주고, 유저 수만큼 보냈다면 끝내기
	
	//sendSelf가 false면 -> 본인한테 보내주는게 아니면 보내야 하는 유저 개수를 
	//수정해줘야 하겠죠 본인한테 보내준 셈 칩시다
	//반복문이 한 명 덜줬는데.. 누구지? 하면서 끝까지 갈 거에요 그거 막아주기
	int send = sendSelf ? 0 : 1;
	//			조건   ? true : false
	//		 본인한테 보냄?  아무한테도 아직 안보냈다! : 1명 보내놨다

	//0번은 Listen Socket	최대치까지 갔거나, 또는 현재 유저 수 만큼 보냈다면
